...
**このサイトは目玉を追わない。** 標準的な図を素直に出し、
**測って捨てた主張の記録そのものを主題**にしている(SPEC §6・2026-09-05 方針転換)。

事前登録した主張を二つ続けて落とした。どちらも閾値を先に置き、外れても動かさず降ろした。

| 主張 | 閾値 | 実測 | 判定 |
|---|---|---|---|
| 陸にいた日が過半 | ≥ 55% | 34.0% | 不成立 |
| 縮んだ版でガラパゴスだけ突出して膨らむ | 順位 ≤ 3 | 4 位 / 21 | 不成立 |

この記録は [`/discarded/`](https://beagle-atlas.vercel.app/discarded/) に残してある。
**消しても他の検査は全部緑のまま通る**ので、消えていないことを実ブラウザ検品で守っている(G-21)。

公開中の画面: 航路と年表(`/voyage/`)・二つの版(`/editions/`)・
測って、捨てた(`/discarded/`)・底本について(`/`)・1845 年版の索引(`/index-gold/`)。
