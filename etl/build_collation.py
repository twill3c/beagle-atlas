...
LEDGER = [
    ...
    },
    {
        "id": "E-04",
        "side": "S-B",
        "where": "本文 第 9 章 玄武岩台地までの距離(段落 c09p009)",
        "observed": "five of six miles",
        "expected": "five or six miles",
        "handling": (
            "黙って書き換えない。**語形そのものが裁定する** —— of では句が成り立たず、"
            "or なら「五、六マイル」という概数として通る。S-A は or と読む。"
            "E-03 と同じく **読み本文が S-B なので、読み手に見えるのはこの誤り** である。"
            "訳は意味の通る側(五、六マイル)を採る —— 綴りの誤りをそのまま日本語に"
            "写す方法がないため。**この一点だけ E-03 と扱いが違う**ので台帳に明記する"
        ),
    },
    {
        "id": "E-05",
        "side": "S-B",
        "where": "本文 第 9 章 フォークランドの狐(段落 c09p032)",
        "observed": "this for will be classed with the dodo",
        "expected": "this fox will be classed with the dodo",
        "handling": (
            "黙って書き換えない。**文脈と語形が裁定する** —— この段落の主語は"
            "Canis antarcticus であり、for では句が成り立たない。S-A は fox と読む。"
            "S-B では行末で fox が for に崩れている(7285-7286 行)。"
            "E-04 と同じく訳は意味の通る側(この狐)を採る。"
            "**S-B 側の異常はこれで三件目**で、E-03(18th←8th)・E-04(of←or)と"
            "合わせて**どれも短い語が一字ないし一語ぶん崩れる形**である。"
            "読み本文の信頼性の傾向としてここに記録しておく"
        ),
    },
    {
        "id": "E-06",
        "side": "S-B",
        "where": "本文 第 13 章 火山の名(段落 c13p007)",
        "observed": "the volcano of Orsono",
        "expected": "the volcano of Osorno",
        "handling": (
            "黙って書き換えない。**この一件は S-A を持ち出さなくても裁定できる** —— "
            "S-B 自身がこの語を Osorno 二回・Orsono 一回と読んでおり、"
            "**読み本文の多数派が自らの外れ値を否定している**。S-A は六箇所すべてで"
            "Osorno と読み、揺れがない。E-04・E-05 と同じく訳は意味の通る側を採る。"
            "**S-B 側の異常はこれで四件目**で、E-03(18th←8th)・E-04(of←or)・"
            "E-05(for←fox)と合わせて、四件とも短い語の一字ないし一語ぶんの崩れ"
            "(ここでは文字の入れ替わり)である。傾向はさらに強まった"
        ),
    },
    {
        "id": "E-07",
        "side": "S-B",
        "where": "本文 第 17 章 植物相の出典参照(段落 c17p042)",
        "observed": "(according to Collnet, p. 58)",
        "expected": "(according to Collnett, p. 58)",
        "handling": (
            "黙って書き換えない。**E-06 と同じく S-A を持ち出さなくても裁定できる** —— "
            "S-B 自身が同じ章の c17p030 で Captain Collnett と読んでおり、"
            "**同一人物の名が同じ章のうちで二通りに綴られている**。S-A は二箇所とも"
            "Collnett と読み、揺れがない。E-04・E-05・E-06 と同じく訳は"
            "意味の通る側(コルネット)を採る。"
            "**S-B 側の異常はこれで五件目**で、E-03(18th←8th)・E-04(of←or)・"
            "E-05(for←fox)・E-06(Orsono←Osorno)と合わせて、五件とも"
            "短い語の一字ないし一語ぶんの崩れである(ここでは末尾の一字の脱落)。"
            "**長い語や文の単位で崩れた例は五件を通して一つも出ていない**"
        ),
    },
]
